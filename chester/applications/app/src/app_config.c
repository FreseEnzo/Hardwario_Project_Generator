...
#define SETTINGS_PFX "chester-example"
...
{
	int ret;
	ret = export_func("chester-example/mode", &m_app_config_interim.mode,
					  sizeof( m_app_config_interim.mode));
	if (ret < 0) {
		return ret;
	}

	ret = export_func("chester-example/interval_sample", &m_app_config_interim.interval_sample,
					  sizeof( m_app_config_interim.interval_sample));
	if (ret < 0) {
		return ret;
	}

	ret = export_func("chester-example/hygro-t-alarm-hi-thr", &m_app_config_interim.hygro_t_alarm_hi_thr,
					  sizeof( m_app_config_interim.hygro_t_alarm_hi_thr));
	if (ret < 0) {
		return ret;
	}

	ret = export_func("chester-example/apn", &m_app_config_interim.apn,
					  sizeof( m_app_config_interim.apn));
	if (ret < 0) {
		return ret;
	}

	ret = export_func("chester-example/hygro-t-alarm-lo-report", &m_app_config_interim.hygro_t_alarm_lo_report,
					  sizeof( m_app_config_interim.hygro_t_alarm_lo_report));
	if (ret < 0) {
		return ret;
	}

	ret = export_func("chester-example/channel-0-differential", &m_app_config_interim.channel_differential[0],
					  sizeof( m_app_config_interim.channel_differential[0]));
	if (ret < 0) {
		return ret;
	}
	
	ret = export_func("chester-example/channel-1-differential", &m_app_config_interim.channel_differential[1],
					  sizeof( m_app_config_interim.channel_differential[1]));
	if (ret < 0) {
		return ret;
	}
	
	ret = export_func("chester-example/channel-2-differential", &m_app_config_interim.channel_differential[2],
					  sizeof( m_app_config_interim.channel_differential[2]));
	if (ret < 0) {
		return ret;
	}
	
	ret = export_func("chester-example/channel-3-differential", &m_app_config_interim.channel_differential[3],
					  sizeof( m_app_config_interim.channel_differential[3]));
	if (ret < 0) {
		return ret;
	}
	
	ret = export_func("chester-example/channel-0-calib-x0", &m_app_config_interim.channel_calib_x0[0],
					  sizeof( m_app_config_interim.channel_calib_x0[0]));
	if (ret < 0) {
		return ret;
	}
	
	ret = export_func("chester-example/channel-1-calib-x0", &m_app_config_interim.channel_calib_x0[1],
					  sizeof( m_app_config_interim.channel_calib_x0[1]));
	if (ret < 0) {
		return ret;
	}
	
	ret = export_func("chester-example/channel-2-calib-x0", &m_app_config_interim.channel_calib_x0[2],
					  sizeof( m_app_config_interim.channel_calib_x0[2]));
	if (ret < 0) {
		return ret;
	}
	
	ret = export_func("chester-example/channel-3-calib-x0", &m_app_config_interim.channel_calib_x0[3],
